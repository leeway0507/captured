...
func (uq *UserQuery) QueryOrderHistories() *OrderHistoryQuery {
	query := (&OrderHistoryClient{config: uq.config}).Query()
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withOrderHistories: uq.withOrderHistories.Clone(),
...
func (uq *UserQuery) WithOrderHistories(opts ...func(*OrderHistoryQuery)) *UserQuery {
	query := (&OrderHistoryClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withOrderHistories = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withOrderHistories; query != nil {
		if err := uq.loadOrderHistories(ctx, query, nodes,
			func(n *User) { n.Edges.OrderHistories = []*OrderHistory{} },
			func(n *User, e *OrderHistory) { n.Edges.OrderHistories = append(n.Edges.OrderHistories, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadOrderHistories(ctx context.Context, query *OrderHistoryQuery, nodes []*User, init func(*User), assign func(*User, *OrderHistory)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(orderhistory.FieldUserID)
	}
	query.Where(predicate.OrderHistory(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.OrderHistoriesColumn), fks...))
	}))
