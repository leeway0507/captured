...
export default function Filter({ initFilterMeta, isOpen, pageType }: FilterDropdownProps) {
    ...
    return (
        <div className={`w-full h-full ${isOpen ? "block" : "hidden"}`} id="accordion-scroll">
            ...
            <AccordionComponent title="사이즈" content={OptionArraySelected(sizeArray, setSizeArray)} cat="sizeArray" />
            <AccordionComponent
                title="배송"
                content={OptionArraySelected(initFilterMeta.intl, setIntl)}
                cat="delivery"
            />
            <AccordionComponent title="가격" content={Slider(initFilterMeta.price, setPrice)} cat="price" />
        </div>
    );
}
